...
// SuffixArrayRune a data type that computes the suffix array of a string
// using 3-way radix quicksort.
type SuffixArrayRune struct {
	text  []rune
	index []int // index[i]=j means text[j:] is ith largest suffix
	n     int   // number of characters in text
}

func NewSuffixArrayRune(txt string) *SuffixArrayRune {
	...
	sa := &SuffixArrayRune{text: txtRune, index: index, n: n}
...
func (sa *SuffixArrayRune) sort(lo, hi, d int) {
...
func (sa *SuffixArrayRune) insertion(lo, hi, d int) {
	for i := lo; i <= hi; i++ {
		for j := i; j > lo; j-- {
...
func (sa *SuffixArrayRune) less(i, j, d int) bool {
	if i == j {
		return false
	}
	i, j = i+d, j+d
...
func (sa *SuffixArrayRune) LCP(i int) int {
	if i < 1 || i >= sa.n {
...
func (sa *SuffixArrayRune) lcp(i, j int) int {
	n := 0
	for i < sa.n && j < sa.n {
		if sa.text[i] != sa.text[j] {
			return n
...
func (sa *SuffixArrayRune) Select(i int) string {
	if i < 0 || i >= sa.n {
		panic("Illegal argument")
	}
	m := sa.index[i]
...
func (sa *SuffixArrayRune) Rank(query string) int {
	lo, hi := 0, sa.n-1
	for lo <= hi {
		mid := lo + (hi-lo)/2
...
// is query < text[i..n)
func (sa *SuffixArrayRune) compare(query string, i int) int {
	queryRune := []rune(query)
	m := len(queryRune)
	j := 0
	for i < sa.n && j < m {
		if queryRune[j] != sa.text[i] {
			return int(queryRune[j]) - int(sa.text[i])
		}
		i++
		j++
	}
