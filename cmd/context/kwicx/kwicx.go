...
// RUN
// go run kwicx.go -f testdata/tale.txt.gz -k 10
// go run kwicx.go -f testdata/rune.txt -k 10
// go run kwicx.go -f testdata/sgyy.txt -k 10
func main() {
	flag.Parse()
	...
	// build suffix array
	sa := suffix.NewSuffixArrayRune(text)
	...
	for {
		...
		for i := sa.Rank(keyword); i < n; i++ {
			from1 := sa.Index(i)
			to1 := min(n, from1+runeLen)
			if keyword != string(txtRune[from1:to1]) {
				break
			}
			from2 := max(0, from1-k)
			to2 := min(n, to1+k)
			fmt.Println(string(txtRune[from2:to2]))
			count++
		}
